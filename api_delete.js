...
exports.delete_article = async (req, res) => {
    ...
    try {
        sql = 'SELECT bucketfolder FROM users WHERE userid = ?; \
               SELECT bucketkey FROM articles WHERE userid=? AND articleid=?; \
               DELETE FROM articles WHERE userid=? AND articleid=?;'
        
        dbConnection.query(sql, [user_id, user_id, article_id, user_id, article_id], async function (err, results) {
            if (err) {
                res.status(400).json({
                    "message": err.message
                });
            ...
            if (results[0].length === 0) {
                ...
                res.status(400).json({
                    "message": "Article not found... Does the inputted article belong to the inputted user?"
                });
                return;
            }
