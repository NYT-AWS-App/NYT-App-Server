    ...
    if (search_filter === "keyword") {
      try {
        ...
        const response = await fetch(
          `https://api.nytimes.com/svc/search/v2/articlesearch.json?q=${search_term}&api-key=${mySecret}`
        );
      ...
      try {
        ...
        const response = await fetch(
          `https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=headline:("${search_term}")&api-key=${mySecret}`
        );
        const nyt_data = await response.json();
      ...
      try {
        ...
        const response = await fetch(
          `https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=byline:("${search_term}")&api-key=${mySecret}`
        );
        const nyt_data = await response.json();
      ...
      try {
        ...
        const response = await fetch(
          `https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=pub_date:("${search_term}")&api-key=${mySecret}`
        );
        const nyt_data = await response.json();
